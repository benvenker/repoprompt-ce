...
public enum WorkspaceFileTreeSnapshotMode: String {
    case none
    case selected
    case full
    case folders
    ...
    public init(fileTreeOption: FileTreeOption) {
...
public struct WorkspaceFileTreeSnapshotRequest {
    fileprivate let selectedFileIDs: Set<UUID>
    public let mode: WorkspaceFileTreeSnapshotMode
    public let filePathDisplay: FilePathDisplay
...
public struct WorkspaceObservedCodemapResult: @unchecked Sendable {
    public let fullPath: String
    public let modificationDate: Date
    public let fileAPI: FileAPI?

    public init(fullPath: String, modificationDate: Date, fileAPI: FileAPI?) {
...
public struct WorkspaceCodemapFileAPIAggregate {
    public let orderedFileAPIs: [FileAPI]
    public let firstFileAPIByStandardizedNestedPath: [String: FileAPI]
}

public enum WorkspaceFileCatalogMaterializationResult: Equatable {
    case materialized(WorkspaceFileRecord)
    case ineligible(CatalogRegularFileIneligibilityReason)

    public var file: WorkspaceFileRecord? {
        if case let .materialized(file) = self { return file }
        return nil
    }

    public var ineligibilityReason: CatalogRegularFileIneligibilityReason? {
        if case let .ineligible(reason) = self { return reason }
        return nil
    }
}

public enum WorkspaceExplicitFileMaterializationResult: Equatable {
    case materialized(WorkspaceFileRecord)
    case noCandidate
    case blocked
    case ambiguous
...
public enum WorkspaceExplicitCatalogFileLookupResult: Equatable {
...
public struct WorkspaceDisplayRootRefsSnapshot: Equatable {
    public let visibleRoots: [WorkspaceRootRef]
    public let allRoots: [WorkspaceRootRef]
}

public actor WorkspaceFileContextStore {
    ...
    private struct ScopedIngressBarrierFlight {
        ...
        public func searchDecodedContentCacheSnapshotForTesting() async -> WorkspaceSearchDecodedContentCache.Snapshot {
            await searchDecodedContentCache.snapshotForTesting()
        }

        public func searchLaneSnapshotForTesting() async -> StoreBackedWorkspaceSearchLane.Snapshot {
            await storeBackedSearchLane.snapshotForTesting()
        }

        public func configureSearchLaneForTesting(
        ...
        public func resetSearchLaneConfigurationForTesting() async -> StoreBackedWorkspaceSearchLane.DebugConfigurationUpdateResult {
    ...
    public init(searchLaneConfiguration: StoreBackedWorkspaceSearchLane.Configuration = .production) {
    ...
    public func roots() -> [WorkspaceRootRecord] {
        rootLoadOrder.compactMap { rootStatesByID[$0]?.root }
    }

    public func rootRecords(forRootFolderPaths rootFolderPaths: [String], includeSystemRoots: Bool = true) -> [WorkspaceRootRecord] {
    ...
    public func fileSystemDeltaEvents() -> AsyncStream<WorkspaceFileSystemDeltaEvent> {
        let streamID = UUID()
    ...
    public func appliedIndexEvents() -> AsyncStream<WorkspaceAppliedIndexBatchEvent> {
        let streamID = UUID()
        return AsyncStream { continuation in
    ...
    public func startWatchingRoot(id rootID: UUID) async throws {
        let state = try state(for: rootID)
    ...
    public func stopWatchingRoot(id rootID: UUID) async {
    ...
    public func replayObservedFileSystemDeltas(rootID: UUID, deltas: [FileSystemDelta]) async {
        ...
        public func appliedIngressSnapshotForTesting(rootID: UUID) -> WorkspaceFileSystemIngressCoordinator.AppliedSnapshot {
    ...
    public func files(inRoot rootID: UUID) -> [WorkspaceFileRecord] {
    ...
    public func folders(inRoot rootID: UUID) -> [WorkspaceFolderRecord] {
        guard let state = rootStatesByID[rootID] else { return [] }
    ...
    public func catalogGeneration(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) -> UInt64 {
        scopedSnapshotGeneration(scope: rootScope)
    }

    public func catalogDiagnostics(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) -> WorkspaceCatalogDiagnostics {
    ...
    public func withStoreBackedSearchAccess<T>(
    ...
    public func rootScopeAvailability(_ rootScope: WorkspaceLookupRootScope) -> WorkspaceLookupRootScopeAvailability {
    ...
    public func searchCatalogAccess(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) -> WorkspaceSearchCatalogAccess {
    ...
    public func searchCatalogSnapshot(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) -> WorkspaceSearchCatalogSnapshot {
    ...
    public func directFolderChildren(
        rootID: UUID,
        relativePath: String = ""
    ...
    public func directFolderChildren(folderID: UUID) -> WorkspaceDirectFolderChildrenSnapshot? {
    ...
    @discardableResult
    public func warmPathLookupIndexes(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) async -> UInt64 {
        while true {
    ...
    public func awaitAppliedIngressForAllRoots() async -> [WorkspaceIngressBarrierSample] {
    ...
    public func awaitAppliedIngress(rootScope: WorkspaceLookupRootScope) async -> [WorkspaceIngressBarrierSample] {
    ...
    public func awaitAppliedIngressForExplicitRequest(
        userPath: String,
    ...
    public func flushPendingServiceEventsForAllRoots() async -> [(rootPath: String, pendingRawEventCountBeforeFlush: Int)] {
    ...
    public func updateDeferredReplayRoutingState(
    ...
    public func updateDeferredReplayImmediateChunkSizeOverride(_ chunkSize: Int?) async {
        await deferredReplayBuffer.updateImmediateReplayChunkSizeOverride(chunkSize)
    }

    public func registerDeferredReplayRootGeneration(_ generation: UInt64, forRootKey rootKey: String) async {
        await deferredReplayBuffer.registerActiveRootGeneration(generation, forRootKey: rootKey)
    }

    public func unregisterDeferredReplayRootGeneration(forRootKey rootKey: String) async {
        await deferredReplayBuffer.unregisterActiveRootGeneration(forRootKey: rootKey)
    }

    public func ingestDeferredReplayLiveDeltas(
        _ deltas: [FileSystemDelta],
    ...
    public func finishDeferredReplayPreparedImmediateIngress(_ immediateReplay: PreparedImmediateReplay) async {
        await deferredReplayBuffer.finishPreparedImmediateIngress(immediateReplay)
    }

    public func enqueueDeferredReplayDeltas(
        _ deltas: [FileSystemDelta],
        forRootKey rootKey: String
    ) async -> DeferredReplayIngressResult {
    ...
    public func drainDeferredReplayPreparedBatches(
    ...
    public func clearDeferredReplayRoot(_ rootKey: String) async {
        await deferredReplayBuffer.clearRoot(rootKey)
    }

    public func clearDeferredReplayBuffer() async {
        await deferredReplayBuffer.clearAll()
    }

    public func hasDeferredReplayPendingWork() async -> Bool {
        await deferredReplayBuffer.hasPendingWork()
    }

    public func pendingDeferredReplayDeltaCount(forRootKey rootKey: String) async -> Int {
        await deferredReplayBuffer.pendingDeltaCount(forRootKey: rootKey)
    }

    public func deferredReplayPendingWorkSnapshot() async -> DeferredReplayPendingWorkSnapshot {
        await deferredReplayBuffer.pendingWorkSnapshot()
    }

    #if DEBUG
        public func deferredReplayDiagnosticsSnapshot() async -> DeferredReplayBufferDiagnostics {
            await deferredReplayBuffer.diagnosticsSnapshot()
        }
    #endif

    public func refreshFileSystemSettings(
        rootID: UUID,
    ...
    public func allCodemapSnapshots() -> [WorkspaceCodemapSnapshot] {
        codemapSnapshotsByFileID.values
            .filter { isDiscoverableFileID($0.fileID) }
            .sorted { $0.fullPath < $1.fullPath }
    }

    public func allCodemapFileAPIs() -> [FileAPI] {
        codemapFileAPIAggregate().orderedFileAPIs
    }

    public func codemapFileAPIAggregate() -> WorkspaceCodemapFileAPIAggregate {
    ...
    public func codemapSnapshotDictionary() -> [UUID: WorkspaceCodemapSnapshot] {
        codemapSnapshotsByFileID.filter { isDiscoverableFileID($0.key) }
    }

    public func codemapSnapshots(inRoot rootID: UUID) -> [WorkspaceCodemapSnapshot] {
    ...
    public func codemapSnapshot(fileID: UUID) -> WorkspaceCodemapSnapshot? {
        guard isDiscoverableFileID(fileID) else { return nil }
        return codemapSnapshotsByFileID[fileID]
    }

    public func codemapSnapshot(rootID: UUID, relativePath: String) -> WorkspaceCodemapSnapshot? {
    ...
    @discardableResult
    public func invalidateCodemapSnapshotsForCheckoutMutation(rootIDs: [UUID]) -> [UUID] {
    ...
    @discardableResult
    public func applyObservedCodemapResults(_ results: [WorkspaceObservedCodemapResult]) -> [String] {
    ...
    @discardableResult
    public func reconcileLoadedRootCatalogWithDisk(rootID: UUID) async -> [FileSystemDelta] {
        guard let state = rootStatesByID[rootID] else { return [] }
        let root = state.root
        let folderPaths = Set(
    ...
    public func ensureIndexedFiles(paths: [String]) async -> [String] {
        var indexed: [String] = []
    ...
    public func makeFileTreeSelectionSnapshot(_ request: WorkspaceFileTreeSnapshotRequest) -> FileTreeSelectionSnapshot {
        makeFileTreeSelectionSnapshot(request, selectedStoreFileIDs: request.selectedFileIDs)
    }

    public func makeFileTreeSelectionSnapshot(
    ...
    public func codemapUpdates() -> AsyncStream<WorkspaceCodemapUpdateEvent> {
    ...
    public func codemapScanProgressUpdates() -> AsyncStream<(Int, Int)> {
        codeScanActor.subscribeToProgress()
    }

    public func cancelAllCodemapScans() async {
        await codeScanActor.cancelAllScans()
    }

    public func cancelCodemapScansForCheckoutMutation(rootIDs: [UUID]) async {
    ...
    public func clearAllCodemapCaches(rootFolders: [String]) async {
        await codeScanActor.clearAllCaches(rootFolders: rootFolders)
        removeAllCodemapSnapshots()
    }

    public func purgeStaleCodemapCaches(keepingRootPaths: [String]) async {
        await codeScanActor.purgeStaleRootCaches(keepingRootPaths: keepingRootPaths)
    }

    #if DEBUG
        public func codemapMemoryCounters() async -> CodeScanActor.CodemapMemoryCounters {
    ...
    public func cancelRootLoad(path: String) {
        let standardizedPath = (path as NSString).standardizingPath
    ...
    public func unloadRoot(id rootID: UUID) async {
        await unloadRoots(ids: [rootID])
    }

    public func unloadRoots(ids rootIDs: [UUID]) async {
    ...
    public func file(rootID: UUID, relativePath: String) -> WorkspaceFileRecord? {
        guard let state = rootStatesByID[rootID] else { return nil }
        let key = StandardizedPath.relative(relativePath)
    ...
    public func folder(rootID: UUID, relativePath: String) -> WorkspaceFolderRecord? {
        guard let state = rootStatesByID[rootID] else { return nil }
        let key = StandardizedPath.relative(relativePath)
        guard let folderID = state.folderIDsByRelativePath[key] else { return nil }
    ...
    public func searchContentSnapshot(for expectedRecord: WorkspaceFileRecord) async throws -> FileSearchContentSnapshot {
        for attempt in 0 ..< 2 {
            try Task.checkCancellation()
    ...
    public func clearSearchDecodedContentCache() async {
    ...
    public func readContentWithDate(
        rootID: UUID,
        relativePath: String,
        workloadClass: ContentReadWorkloadClass = .unspecified
    ...
    public func requestCodemapScans(for files: [WorkspaceFileRecord]) async throws {
        ensureCodeScanResultTask()
    ...
    @discardableResult
    public func createFile(rootID: UUID, relativePath: String, content: String) async throws -> WorkspaceFileCatalogMaterializationResult {
        let state = try state(for: rootID)
    ...
    @discardableResult
    public func editFile(rootID: UUID, relativePath: String, newContent: String) async throws -> WorkspaceFileCatalogMaterializationResult? {
        let state = try state(for: rootID)
        let standardizedRelativePath = StandardizedPath.relative(relativePath)
        do {
    ...
    public func moveFile(rootID: UUID, from oldRelativePath: String, to newRelativePath: String) async throws {
        let state = try state(for: rootID)
    ...
    public func deleteFile(rootID: UUID, relativePath: String) async throws {
        let state = try state(for: rootID)
        let standardizedRelativePath = StandardizedPath.relative(relativePath)
    ...
    public func moveItemToTrash(rootID: UUID, relativePath: String) async throws {
        let state = try state(for: rootID)
        let standardizedRelativePath = StandardizedPath.relative(relativePath)
        let oldFile = file(rootID: rootID, relativePath: standardizedRelativePath)
        let oldFileWasDiscoverable = oldFile.map { isDiscoverableFileID($0.id) } ?? false
    ...
    public func validateCatalogFileStillPresent(_ file: WorkspaceFileRecord) async -> WorkspaceFileRecord? {
        let lifecycleCorrelation = EditFlowPerf.currentLifecycleCorrelation
        EditFlowPerf.lifecycleEvent(
    ...
    @discardableResult
    public func pruneMissingCatalogFilesForExactMutationLookup(
        _ userPath: String,
    ...
    public func lookupCatalogFileForExplicitRequest(
        _ userPath: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace
    ...
    public func materializeExplicitlyRequestedFile(
        _ userPath: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace
    ...
    @discardableResult
    public func materializeCatalogFileAfterDiskWrite(
        rootID: UUID,
        relativePath: String
    ...
    public func lookupPath(_ request: WorkspacePathLookupRequest) async -> WorkspacePathLookupResult? {
    ...
    public func lookupPaths(_ requests: [WorkspacePathLookupRequest]) async -> [String: WorkspacePathLookupResult] {
    ...
    public func resolveCreationPath(
        userPath: String,
        rootScope: WorkspaceLookupRootScope = .allLoaded,
        selectedFileFullPaths: Set<String> = [],
    ...
    public func lookupPath(rootID: UUID, relativePath: String) -> WorkspacePathLookupResult? {
        guard let state = rootStatesByID[rootID] else { return nil }
        let key = StandardizedPath.relative(relativePath)
    ...
    public func lookupDiscoverableCatalogPathForExactAbsoluteSearchScope(
        _ userPath: String,
        rootScope: WorkspaceLookupRootScope
    ...
    public func rootRefs(scope: WorkspaceLookupRootScope = .allLoaded) -> [WorkspaceRootRef] {
        rootsForPathLookup(scope: scope).map {
            WorkspaceRootRef(id: $0.id, name: $0.name, fullPath: $0.standardizedFullPath)
        }
    }

    public func displayRootRefsSnapshot() -> WorkspaceDisplayRootRefsSnapshot {
    ...
    public func exactPathResolutionIssue(
        for userPath: String,
    ...
    public func resolveFolderInput(
        _ path: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace,
        profile: PathLocateProfile = .mcpSelection
    ...
    public func expandFolderInputToFiles(
        _ path: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace,
        profile: PathLocateProfile = .mcpSelection
...
public enum WorkspaceFileContextStoreError: Error, Equatable {
