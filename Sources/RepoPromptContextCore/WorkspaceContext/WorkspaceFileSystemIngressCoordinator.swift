...
public final class WorkspaceFileSystemIngressCoordinator: @unchecked Sendable {
    public struct Subscription: Hashable {
        let rootID: UUID
        let generation: UInt64
    }

    public struct AppliedSnapshot: Equatable {
    ...
    public typealias DrainHandler = @Sendable (FileSystemDeltaPublication, EditFlowPerf.LifecycleCorrelation?) async -> Void
    ...
    public func openPublisherIngress(rootID: UUID, drainHandler: @escaping DrainHandler) -> Subscription {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func closePublisherIngress(rootID: UUID) {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func isPublisherIngressOpen(_ subscription: Subscription) -> Bool {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func hasOpenPublisherIngress(rootID: UUID) -> Bool {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func waitUntilApplied(rootID: UUID, servicePublicationSequence: UInt64) async {
    ...
    public func waitForCurrentPublisherIngress(rootIDs: Set<UUID>) async {
    ...
    public func appliedSnapshot(rootID: UUID) -> AppliedSnapshot {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func pendingPublisherIngressCount(rootIDs: Set<UUID>) -> Int {
        lock.lock()
        defer { lock.unlock() }
    ...
    public func finishPublisherIngress(rootIDs: Set<UUID>) {
