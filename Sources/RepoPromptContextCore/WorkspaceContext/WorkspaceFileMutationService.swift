...
public enum WorkspaceFileCreatePathResolutionPolicy {
    case literalPreferredIfStronger
    case canonicalAliasFirst
}

public struct WorkspaceFileMutationWriteResult: Equatable {
    public let diskSucceeded: Bool
    public let materializedFile: WorkspaceFileRecord?
    public let catalogIneligibility: CatalogRegularFileIneligibilityReason?

    public var catalogMaterialized: Bool {
        materializedFile != nil
    }

    public static func fromCatalogMaterialization(_ result: WorkspaceFileCatalogMaterializationResult) -> WorkspaceFileMutationWriteResult {
        switch result {
...
public struct WorkspaceFileMutationService {
    ...
    public func resolveExactExistingFileForMutation(
        _ userPath: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace
    ...
    public func readText(file: WorkspaceFileRecord) async throws -> String? {
        try await store.readContent(rootID: file.rootID, relativePath: file.standardizedRelativePath)
    }

    @discardableResult
    public func overwrite(file: WorkspaceFileRecord, content: String) async throws -> WorkspaceFileMutationWriteResult {
    ...
    public func createFileWithPostcondition(
        userPath: String,
        content: String,
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace,
        selectedFileFullPaths: Set<String> = [],
