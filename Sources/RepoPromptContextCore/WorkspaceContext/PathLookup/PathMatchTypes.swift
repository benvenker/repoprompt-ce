...
public struct PathMatchLocation {
...
public struct StaticPathMatchData {
...
/// All references to UI/ViewModel types are stripped; uses frozen records only.
public struct PathMatchSnapshot {
    public let filesByFullPath: [String: FileRecord]
    public let foldersByFullPath: [String: FolderRecord]
    public let rootFolders: [FolderRecord]
    ...
    public var indexes: PathMatchIndexes {
        storedIndexes
    }
    ...
    public func fileRecord(forFullPath path: String) -> FileRecord? {
        let std = (path as NSString).standardizingPath
        return fileRecord(forStandardizedFullPath: std)
    }

    public func fileRecord(forStandardizedFullPath path: String) -> FileRecord? {
        filesByFullPath[path] ?? filesByLowerFullPath[path.lowercased()]
    }

    public func folderRecord(forFullPath path: String) -> FolderRecord? {
        let std = (path as NSString).standardizingPath
        return folderRecord(forStandardizedFullPath: std)
    }

    public func folderRecord(forStandardizedFullPath path: String) -> FolderRecord? {
...
public struct PathMatchIndexes {
...
public enum PathLocateProfile: Hashable {
    case uiAssisted
    case mcpRead
    case mcpSelection
    case mcpSearchScope
    ...
    public var options: PathLocateOptions {
        switch self {
        case .uiAssisted:
            PathLocateOptions(
...
public struct PathLocateOptions: Equatable {
    ...
    public let useSelectedRootBias: Bool
}

/// Result of finding a path for file creation
public struct FileCreationResult {
...
extension FileCreationResult: Equatable {
    public static func == (lhs: FileCreationResult, rhs: FileCreationResult) -> Bool {
...
public enum CreationResolutionMode {
...
public enum FileCreationResolution: Equatable {
