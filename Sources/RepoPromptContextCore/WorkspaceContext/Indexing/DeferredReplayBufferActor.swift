...
public protocol DeltaReplayPreparing: Actor {
    func prepare(
        rootKey: String,
...
public struct PreparedImmediateReplay {
    ...
    public let rootGeneration: UInt64
    public let sourceDeltas: [FileSystemDelta]
    public let preparedBatch: PreparedFileSystemReplayBatch
    public let routingVersion: UInt64
...
public enum DeferredReplayIngressResult {
...
public struct DeferredReplayPendingWorkSnapshot: Equatable {
    public let pendingRootCount: Int
    public let pendingDeltaCount: Int
    public let overflowedRootCount: Int
}

#if DEBUG
    public struct DeferredReplayBufferDiagnostics: Equatable {
...
public actor DeferredReplayBufferActor {
    ...
    public func updateRoutingState(
    ...
    public func updateImmediateReplayChunkSizeOverride(_ chunkSize: Int?) {
        if let chunkSize {
    ...
    public func registerActiveRootGeneration(_ generation: UInt64, forRootKey rootKey: String) {
        activeRootGenerations[rootKey] = generation
        clearBufferedState(forRootKey: rootKey)
    }

    public func unregisterActiveRootGeneration(forRootKey rootKey: String) {
        activeRootGenerations.removeValue(forKey: rootKey)
        clearBufferedState(forRootKey: rootKey)
    }

    public func ingestLiveDeltas(
    ...
    public func finishPreparedImmediateIngress(_ immediateReplay: PreparedImmediateReplay) {
        releaseImmediatePreparedIngressReservation(immediateReplay.reservation)
    }

    public func enqueueDeferredDeltas(
        _ deltas: [FileSystemDelta],
        forRootKey rootKey: String
    ...
    public func drainPreparedBatches(
    ...
    public func clearRoot(_ rootKey: String) {
        clearBufferedState(forRootKey: rootKey)
    }
    ...
    public func pendingDeltaCount(forRootKey rootKey: String) -> Int {
        pendingDeltasByRoot[rootKey]?.count ?? 0
    }

    public func pendingWorkSnapshot() -> DeferredReplayPendingWorkSnapshot {
