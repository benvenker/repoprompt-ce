...
public enum RepoFileReplayPerf {
    #if (DEBUG || EDIT_FLOW_PERF) && canImport(os)
        public typealias State = OSSignpostIntervalState
        private static let signposter = OSSignposter(subsystem: "com.repoprompt.workspace", category: "file-replay")
        public static var isEnabled: Bool {
            UserDefaults.standard.bool(forKey: "enableRepoFileReplaySignposts")
        }

        public static func begin(_ name: StaticString) -> State? {
            guard isEnabled else { return nil }
            return signposter.beginInterval(name)
        }

        public static func end(_ name: StaticString, _ state: State?) {
...
public struct PreparedFileSystemDelta {
    public let delta: FileSystemDelta
    public let relativePath: String
    public let absolutePath: String

    public var isFolderAdded: Bool {
        if case .folderAdded = delta { return true }
        return false
    }

    public var isFolderRemoved: Bool {
        if case .folderRemoved = delta { return true }
        return false
    }
}

public struct PreparedFolderRenameTransfer: Equatable {
    public let oldAbsolutePath: String
    public let newAbsolutePath: String
}

public struct PreparedFileSystemReplayChunkSummary: Equatable {
    public var fileAddedCount: Int = 0
    public var fileRemovedCount: Int = 0
    public var folderAddedCount: Int = 0
    public var folderRemovedCount: Int = 0
...
public struct PreparedFileSystemReplayChunk: Equatable {
...
public struct PreparedFileSystemReplayBatch {
    public let rootKey: String
    public let queuedDeltaCount: Int
    public let coalescedDeltaCount: Int
    public let preparedDeltas: [PreparedFileSystemDelta]
...
public enum FileSystemDeltaPreparation {
    private static func timestampMS() -> Double {
        ProcessInfo.processInfo.systemUptime * 1000
    }

    public static func rawRelativePath(for delta: FileSystemDelta) -> String {
        switch delta {
    ...
    public static func standardizedRelativePath(for delta: FileSystemDelta) -> String {
        StandardizedPath.relative(rawRelativePath(for: delta))
    }

    public static func containedPaths(
    ...
    public static func makeChunkRanges(totalCount: Int, chunkSize: Int) -> [Range<Int>] {
...
public actor DeltaReplayPreparationActor: DeltaReplayPreparing {
    public init() {}
