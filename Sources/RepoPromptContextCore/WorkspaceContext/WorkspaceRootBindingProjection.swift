...
public struct WorkspaceRootBindingProjection: Equatable {
    public let sessionID: UUID
    public let replacementsByLogicalRootPath: [String: BoundRoot]
    private let visibleLogicalRoots: [WorkspaceRootRef]

    public struct BoundRoot: Equatable {
        public let logicalRoot: WorkspaceRootRef
        public let physicalRoot: WorkspaceRootRef
        public let binding: AgentSessionWorktreeBinding

        public init(logicalRoot: WorkspaceRootRef, physicalRoot: WorkspaceRootRef, binding: AgentSessionWorktreeBinding) {
    ...
    public init(sessionID: UUID, boundRoots: [BoundRoot], visibleLogicalRoots: [WorkspaceRootRef] = []) {
    ...
    public func translateInputPath(_ rawPath: String) -> String {
    ...
    public func translateInputPaths(_ paths: [String]) -> [String] {
        paths.map { translateInputPath($0) }
    }

    public func translateSliceInputs(_ slices: [WorkspaceSelectionSliceInput]) -> [WorkspaceSelectionSliceInput] {
        slices.map { input in
    ...
    public func projectedLogicalDisplayPath(forPhysicalPath rawPath: String, display: FilePathDisplay = .relative) -> String? {
    ...
    public func logicalDisplayPath(forPhysicalPath rawPath: String, display: FilePathDisplay = .relative) -> String {
    ...
    public func logicalizeFileTreeSnapshot(_ snapshot: FileTreeSelectionSnapshot) -> FileTreeSelectionSnapshot {
    ...
    public func logicalizeSelection(_ selection: StoredSelection) -> StoredSelection {
    ...
    public func physicalizeSelection(_ selection: StoredSelection) -> StoredSelection {
        var slices: [String: [LineRange]] = [:]
        for (path, ranges) in selection.slices {
...
public struct WorkspaceRootBindingProjectionMaterializer {
    public let store: WorkspaceFileContextStore

    public init(store: WorkspaceFileContextStore) {
        self.store = store
    }
...
public struct WorkspaceLookupContext: Equatable {
    public let rootScope: WorkspaceLookupRootScope
    public let bindingProjection: WorkspaceRootBindingProjection?

    public init(rootScope: WorkspaceLookupRootScope, bindingProjection: WorkspaceRootBindingProjection?) {
        self.rootScope = rootScope
        self.bindingProjection = bindingProjection
    }

    public static let visibleWorkspace = WorkspaceLookupContext(rootScope: .visibleWorkspace, bindingProjection: nil)

    public func translateInputPath(_ path: String) -> String {
    ...
    public func displayPath(forPhysicalPath path: String, display: FilePathDisplay = .relative) -> String {
