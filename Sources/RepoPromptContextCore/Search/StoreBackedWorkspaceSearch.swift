...
public enum StoreBackedWorkspaceSearchError: LocalizedError, Equatable {
...
public enum StoreBackedWorkspaceSearch {
    public static func search(
        ...
        searchReadiness: (@Sendable () async -> WorkspaceSearchReadinessState?)? = nil
    ) async throws -> SearchResults {
        try Task.checkCancellation()
        try await ensureRootScopeAvailable(rootScope, store: store)
        try await ensureSearchReady(store: store, searchReadiness: searchReadiness)
    ...
    public static func requiresBroadSearchAdmission(
        pattern: String,
        mode: SearchMode,
        paths: [String]?
    ) -> Bool {
    ...
    public static func broadSearchAdmissionClass(
        pattern: String,
        mode: SearchMode,
        paths: [String]?
    ...
    private static func ensureSearchReady(
        store: WorkspaceFileContextStore,
        searchReadiness: (@Sendable () async -> WorkspaceSearchReadinessState?)?
    ) async throws {
        ...
        guard let searchReadiness else { return }
        guard let state = await searchReadiness() else { return }
        switch state {
        case .ready, .degraded:
            return
        case .idle:
