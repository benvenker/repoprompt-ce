...
public struct CodeMapPCRE2Match {
    private let captures: [String?]

    public init(subject: String, match: PCRE2Match) {
    ...
    public func trimmedCapture(_ index: Int) -> String? {
        capture(index)?.trimmingCharacters(in: .whitespaces)
    }
}

public struct CodeMapPCRE2Pattern {
    ...
    public func firstMatch(in text: String) -> CodeMapPCRE2Match? {
        guard let match = try? regex.firstMatch(in: text) else { return nil }
        return CodeMapPCRE2Match(subject: text, match: match)
    }

    public func firstCapture(_ index: Int = 1, in text: String) -> String? {
        firstMatch(in: text)?.capture(index)
    }

    public func trimmedCapture(_ index: Int = 1, in text: String) -> String? {
    ...
    public func wholeMatch(in text: String) -> Bool {
