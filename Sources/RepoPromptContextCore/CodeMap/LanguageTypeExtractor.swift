...
public enum LanguageTypeExtractor {
    // MARK: - Swift Patterns
    ...
    public static let swiftFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^
    (?:[-*]\s*)?
    \s*
    ...
    public static let swiftVariableRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^
    (?:[-*]\s*)?
    \s*
    ...
    public static let cSharpVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    public static let cSharpFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:public|private|protected|internal)?\s*
    ...
    public static let javaVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    public static let javaFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:public|private|protected)?\s*
    ...
    public static let dartVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:
     (?:final|const)\s+
    ...
    public static let dartFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    (?:
    ...
    public static let dartFactoryRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    factory\s+
    ...
    public static let dartGetterSetterRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    ...
    public static let cVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    public static let cFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:extern|static)?\s*
    ...
    public static let cppVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    public static let cppFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:template\s*<[^>]*>\s*)?
    ...
    public static let cppConstructorRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    (?:template\s*<[^>]*>\s*)?
    ...
    public static let cppTrailingReturnFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:template\s*<[^>]*>\s*)?
    (?:inline\s+|static\s+|virtual\s+|constexpr\s+|const\s+|unsigned\s+|signed\s+|long\s+|short\s+|typename\s+|friend\s+)*
    ...
    public static let pythonVariableRegex: Regex<(Substring, Substring, Substring)> =
    ...
    public static let pythonFunctionRegex: Regex<(Substring, Substring, Substring, Substring?)> =
    ...
    /// Output: (wholeMatch, varName, typeName?)
    public static let jsTsVariableRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:var|let|const)\s+([A-Za-z_]\w*\??)\s*(?::\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?"#)

    /// JS/TS function: `(export)? (default)? (async)? function name<T>?(params): ReturnType?`
    /// Output: (wholeMatch, funcName, paramList, returnType?)
    public static let jsTsFunctionRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?function\s+([A-Za-z_]\w*\??)(?:<[^>]+>)?\s*\(([^)]*)\)(?:\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?(?:\s*\{)?"#)

    /// TS arrow function pattern
    public static let tsArrowFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    ...
    /// Output: (wholeMatch, funcName, paramList, returnType)
    public static let tsArrowFunctionParamsReturnRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:var|let|const)\s+([A-Za-z_]\w*\??)\s*=\s*\(([^)]*)\)\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-\s,]*)\s*=>"#)

    /// TS type alias: `(export)? type Name = Type`
    /// Output: (wholeMatch, aliasName, rhsType)
    public static let tsTypeAliasRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?type\s+([A-Za-z_]\w*)\s*=\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*)"#)

    /// TS variable: `(export)? (var|let|const) name?: Type`
    /// Output: (wholeMatch, varName, typeName?)
    public static let tsVariableRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:var|let|const)\s+([A-Za-z_]\w*\??)(?:\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?"#)

    // MARK: - TSX

    /// Very similar to TS but we allow a generic in the name, e.g. "function FooComponent<T>()"
    public static let tsxVariableRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    (?:var|let|const)\s+
    ...
    public static let tsxFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    public function\s+
    ...
    public static let tsxArrowFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    (?:var|let|const)\s+
    ...
    public static let tsClassMethodRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    public static let tsClassPropertyRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    public static let tsClassArrowMethodRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    public static let tsClassArrowNoParensRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    (?:(?:public|private|protected|static|readonly|abstract|async)\s+)*
    ...
    public static let tsConstructorRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:(?:public|private|protected)\s+)?
    constructor\s*\(
    ...
    public static let tsAccessorRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    public static let goVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:var|const)\s+
    ([A-Za-z_]\w*)
    ...
    public static let goFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    public func\s+
    (?:\([^)]*\)\s+)?        # optional receiver
    ...
    public static let rustVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    public let\s+(?:mut\s+)?
    ([A-Za-z_]\w*)
    ...
    public static let rustFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    public struct FunctionLineMatch {
        let name: String?
    ...
    public typealias RegexMatch = Regex<AnyRegexOutput>.Match
