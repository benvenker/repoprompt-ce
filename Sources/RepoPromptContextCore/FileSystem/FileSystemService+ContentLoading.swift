...
private func foundationHeuristicEncoding(for data: Data) -> String.Encoding? {
    #if canImport(Darwin)
        ...
        return guess != 0 ? .init(rawValue: guess) : nil
    #else
        return nil
    #endif
}
...
private func detectEncodingFull(_ data: Data) -> String.Encoding {
    ...
    return foundationHeuristicEncoding(for: data) ?? .utf8
}
...
public actor ContentReadAsyncLimiter {
    #if DEBUG
        public struct Snapshot: Equatable {
            public let capacity: Int
            public let maxQueuedWaiterCount: Int
            public let activePermitCount: Int
            public let queuedWaiterCount: Int
            public let ownerLaneCount: Int
            public let cancellationCount: Int
            public let grantCount: Int
            public let overloadCount: Int
            public let interactiveGrantCount: Int
            public let normalGrantCount: Int
            public let bulkGrantCount: Int
    ...
    #if DEBUG
        public func snapshotForTesting() -> Snapshot {
...
extension FileSystemService {
    ...
    #if DEBUG
        public nonisolated static var contentReadWorkerLimitForTesting: Int {
            contentReadWorkerLimit
        }

        public nonisolated static func contentReadWorkerLimiterSnapshotForTesting() async -> ContentReadAsyncLimiter.Snapshot {
    ...
    public func loadContentWithDate(
        ofRelativePath relativePath: String,
        workloadClass: ContentReadWorkloadClass = .unspecified
    ...
    public func loadEntireFileContentOptimized(
        ofRelativePath relativePath: String,
    ...
    public static let orderedFallbackEncodings: [String.Encoding] = [
        .windowsCP1252,
        .macOSRoman
    ]

    /// Optional, low‑priority locale‑specific single‑byte encodings
    public static let regionSpecificEncodings: [String.Encoding] = {
        ...
        encodings.append(contentsOf: [.windowsCP1251, .isoLatin2]) // Cyrillic / Central-Europe
        return encodings
    }()
    ...
    public static let alwaysBinaryExtensions: Set<String> = [
        // ── Video ───────────────────────────────────────────────────
    ...
    public static let alwaysTextExtensions: Set<String> = [
        // ── General text / docs ─────────────────────────────────────
    ...
    public static let alwaysTextFilenames: Set<String> = [
    ...
    private nonisolated static func detectFileEncoding(in data: Data) -> String.Encoding {
        if let heuristicEncoding = foundationHeuristicEncoding(for: data) {
