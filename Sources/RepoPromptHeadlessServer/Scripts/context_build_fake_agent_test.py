...
def run_context(fake_agent, extra_env=None):
    env = dict(os.environ)
    ...
    return subprocess.run(
        [binary, "context-build", "--root", root, "--agent", "fake", "--instructions", "test", "--timeout", "30"],
...
try:
    ...
    assert "oracle_send" not in ok.stdout.split("selection", 1)[0], ok.stdout
